 ...
 */
[Event(name="taskQueueCompleted",  type="loaderqueue.LoaderQueueEvent")]

/**
 * 任务队列及下载队列的管理器. 
 * 使用范例：
 * var urlLoader:URLLoaderAdapter = new URLLoaderAdapter(4,"http://ggg.ggg.com/a.swf");
 * urlLoader.addEventListener(LoaderQueueEvent.TASK_COMPLETED, onLoadedCompleted);
 * 
...
public class LoaderQueue extends EventDispatcher implements ILoaderQueue
{
     ...
     */
    public function LoaderQueue(threadLimit:uint = 2, delay:int = 500, jumpQueueIfCached:Boolean = true)
    {
        ...
        timeOutToSort.addEventListener(TimerEvent.TIMER_COMPLETE, timeOutToSort_timerCompleteHandler);
    }
    ...
    {
        dispatchEvent(new LoaderQueueEvent(LoaderQueueEvent.TASK_ADDED, loaderAdapter));
        ...
        loaderAdapter.addEventListener(LoaderQueueEvent.TASK_DISPOSE, loaderAdapter_disposeHandler);
    ...
    */
    public function dispose():void
    {
        ...
        timeOutToSort.stop();
        timeOutToSort.removeEventListener(TimerEvent.TIMER_COMPLETE, timeOutToSort_timerCompleteHandler);
        timeOutToSort = null;
    }
    ...
    public function removeItem(loaderAdapter:ILoaderAdapter):void
    {
        ...
        dispatchEvent(new LoaderQueueEvent(LoaderQueueEvent.TASK_REMOVED, loaderAdapter));
    }
    ...
    private function checkQueueHandle():Boolean
    {
        ...
        dispatchEvent(new LoaderQueueEvent(LoaderQueueEvent.TASK_QUEUE_COMPLETED));
        return false;
    }
    ...
    private function disposeItem(loaderAdapter:ILoaderAdapter):void
    {
        ...
        {
            loaderAdapter.removeEventListener(LoaderQueueEvent.TASK_COMPLETED, loaderAdapter_completedHandler);
            loaderAdapter.removeEventListener(LoaderQueueEvent.TASK_ERROR, loaderAdapter_errorHandler);
            try
            {
                loaderAdapter.stop();
            }
        ...
        loaderAdapter.removeEventListener(LoaderQueueEvent.TASK_DISPOSE, loaderAdapter_disposeHandler);
    ...
    {
        loaderAdapter.addEventListener(LoaderQueueEvent.TASK_COMPLETED, loaderAdapter_completedHandler, false, 0xFFFFFF, false);
        loaderAdapter.addEventListener(LoaderQueueEvent.TASK_ERROR, loaderAdapter_errorHandler, false, 0xFFFFFF, false);
        loaderAdapter.start();
    }
    ...
    private function threadFullHandle(loaderAdapter:ILoaderAdapter):void
    {
        ...
        {
            var reversePriorityResult:Boolean = checkReversePriority(ILoaderAdapter(threadLib[i]).priority, loaderAdapter.priority);
    ...
    {
        var loaderAdapter:ILoaderAdapter = event.currentTarget as ILoaderAdapter;
